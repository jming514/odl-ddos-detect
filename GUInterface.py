...
def listen():
    def collectdata():
        ...
        def kmeans():
            kmeans = KMeans(
                n_clusters=2)  # creating 2 clusters for ddos/notddos
        ...
        def Randomforest(x1):
            rfc = RandomForestClassifier(
                n_estimators=200)  # how many trees in forest
        ...
        def collectData():
            ...
            for i in range(numOfCon):
                ...
                if a == "openflow:2:1":
                    ...
                    df_out.to_csv(
                        "flowDataset5.csv", index=False)  # write to new file
                    # read new file
    ...
    a_thread = threading.Thread(target=collectdata())
    a_thread.start()
...
def about():
    tkinter.messagebox.showinfo(
        "Live Graph",
        "This program collects network data from OpenDayLight controller and determines if the network flows are normal or DDoS attacks"
    )
...
while (1):
    ...
    quit_button_window = canvas.create_window(
        36, 119, anchor='nw', window=quit_button)
    main_button = tk.Button(
        ...
        bg="#2FB19F",
        activebackground="#00FF00")
    main_button_window = canvas.create_window(
        37, 89, anchor='nw', window=main_button)
    ...
    main2_button_window = canvas.create_window(
        37, 59, anchor='nw', window=main2_button)
